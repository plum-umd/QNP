...
{\footnotesize
\begin{mathpar}
\texttt{pat}(c,i,j) = \ket{0}^{\otimes i}\ket{1}^{\otimes (j-i)}
\\
q(j)=\schai{2^{(\snext{j})}\,\sminus\,2}{z_i}{i}{\ket{\texttt{pat}(c,i,j)}\ket{u_i}\ket{\texttt{key}(i)}} + \ssum{k=j}{2^t}{z_k\ket{k}\ket{\overline{0}}\ket{0}\ket{\overline{0}}}
\\
\texttt{is\_steps}(j,q(j))=
\forall i\in [1,j]. \ket{i}\ket{\texttt{pat}(c,i,j)}\ket{u_i}\ket{\texttt{key(i)}} \in q(j)
