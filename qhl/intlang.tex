...
\begin{figure}[t]
{
  ...
  \[\begin{array}{llcl} 
      ...
      \text{Quantum State} & q & ::= & \ket{c}\mid \shad{2^n}{n}{\alpha(r_j)} \mid \sch{m}{z_j}{c_j} \\
    \end{array}
  \]
}
...
\begin{center}
 \[
  \begin{array}{r@{~}c@{~}l@{~}cl}
  &&\{\bot:q\} \cup \varphi &\equiv& \varphi\\
  q\equiv_{\slen{\lambda}}q' &\Rightarrow& \{\lambda:q\} \cup \varphi &\equiv& \{\lambda:q'\} \cup \varphi\\
  &&\{\lambda_1\uplus l_1 \uplus l_2 \uplus \lambda_2 :q\} \cup \varphi &\equiv& \{\lambda_1\uplus l_2 \uplus l_1 \uplus \lambda_2 : \texttt{mut}(q,\slen{\lambda_1})\} \cup \varphi\\
...
\\
\texttt{spt}(\sch{m}{z_j}{c_j.c},n)=(\sch{m}{z_j}{c_j},\ket{c})
\end{array}
\]
}
\caption{Semantic state equivalence. We use set union ($\cup$) to describe the state concatenation with the empty set operation $\emptyset$. $i$ is a single bit either $0$ or $1$. The $.$ operation is bitstring concatenation. 
Term $\Msum^{n*m}P$ is a summation formula that omits the indexing details.
Term $(\frac{1}{\sqrt{2^{n}}}\Motimes_{j=0}^{n}q_j) \bigotimes(\frac{1}{\sqrt{2^{m}}}\Motimes_{j=0}^{m}q_j)$ is equivalent to $\frac{1}{\sqrt{2^{n+m}}}\Motimes_{j=0}^{n}q_j$.
}
...
{\Small
  \begin{mathpar}
    ...
    \inferrule[SLOOP]{ n_1 < n_2 }
                  {(\varphi,\sqwhile{\sint{x}{n_1}}{x<n_2}{b}{\dplus{x}}{s}) \longrightarrow (\varphi,\sseq{\sifq{b}{s}}{\sqwhile{\sint{x}{\texttt{S}\;n_1}}{x<n_2}{b}{\dplus{x}}{s}})}
